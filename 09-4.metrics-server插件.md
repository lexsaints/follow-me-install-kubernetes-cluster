...
### kube-apiserver
#修改配置参数之后，需要在kube-apiserver部署的步骤里，重新生成配置文件，然后下发到各个节点，重启kube-apiserver服务
添加如下配置参数：
...
$ diff metrics-server-deployment.yaml metrics-server-deployment.yaml.orig
#metrics-server-deployment.yaml配置文件使用上面的镜像地址,可以成功拉下image
