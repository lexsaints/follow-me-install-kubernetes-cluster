...
``` bash
$ export NODE_IP=192.168.172.151 # 当前部署 harbor 的节点 IP
$
```
...
将kubernetes集群的ca.pem ca-key.pem ca-config.json 文件拷贝到harbor服务器的/etc/kubernetes/cert 中
生成 harbor 证书和私钥：
...
浏览器访问 `https://${NODE_IP}`，示例的是 `https://192.168.172.151`；
...
将签署 harbor 证书的 CA 证书拷贝到 `/etc/docker/certs.d/192.168.172.151` 目录下

``` bash
$ sudo mkdir -p /etc/docker/certs.d/192.168.172.151
$ sudo cp /etc/kubernetes/cert/ca.pem /etc/docker/certs.d/192.168.172.151/ca.crt
$
```
